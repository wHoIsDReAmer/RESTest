...
use restest::request::{RequestConfig, Requester, simple_requester::SimpleRequester};
...
async fn test_requester_with_query() {
    ...
    let config = RequestConfig::new().with_query(query);
...
async fn test_requester_with_body() {
    ...
    let config = RequestConfig::new().with_body(body.clone());
...
async fn test_requester_with_timeout() {
    let requester = SimpleRequester {};
    let config = RequestConfig::new().with_timeout(Duration::from_millis(1));
